...
const saveVideoIntroCourse = async (req, res) => {
    try {
        let namevideoIntro = req.file.originalname;

        const filePath = saveVideoBuffer(req.file.buffer, namevideoIntro);
        ...
        res.status(200).json({
            success: true,
            message: "Upload video giới thiệu khóa học thành công",
            namevideoIntro: fileNameWithExt,
        });
    } catch (error) {
        res.status(500).json({ message: "Lỗi saveVideoIntroCourse: " + error.message });
    }
};

module.exports = {
    saveVideoLesson,saveVideoIntroCourse
};
